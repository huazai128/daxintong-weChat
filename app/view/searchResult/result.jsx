		...
		return (
			<div className={`${styles.shop} mb20`} key={sectionID}>
				<div className={styles.head} >
					<SubViewLink moduleName="business" title="商家详情" modelData={{ id: id, store: store }} confirmButtonHtml={<ConfirmHtml id={id} isCollect={is_collect} />}>
						...
						<div className="flex jc-between">
							...
							<p className={styles.ju}>{meter(distance)}</p>
						</div>
					</SubViewLink>
				</div>
...
export default class extends Component {
	constructor(props) {
		super(props);
		this.store = props.modelData.result;
		this.state = {
			height: document.documentElement.clientHeight
		};
	}
	componentDidMount() {
		const { keyword } = this.props.modelData;
		this.store.onChange(keyword, 'keyword');
		this.store.setLoc();
		this.store.getData(keyword);
	}
	...
	render() {
		...
		return (
			<div className="flex-col">
				<div style={{ backgroundColor: '#fff' }} className="flex borderTop borderBottom">
